...
let totalPopulation  = populations.reduce(function(total,element) { return total + element})
...
let ordersTotal = orders.map(function(element,index,arr){return arr[index].price * (1 + arr[index].tax)})
ordersTotal
...
let bobsTotal = purchases.filter(function(element, i, arr){if (arr[i].owner === "Bob"){return arr[i].price}}).map(function(element,i,arr){return arr[i].price}).reduce(function(total,element){return total+=element})
